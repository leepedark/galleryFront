...
<!--                  {{ lib.getNumberFormatted(i.price - i.price * i.discount_per / 100) }}원-->
                  {{ i.price - (i.price * i.discount_per / 100) }}
                </span>
              </li>
            </ul>
  ...
  setup() {
    ...
    const computedPrice = computed(() => {
      ...
      for (let i of state.items) {
        result += i.price - (i.price * i.discount_per / 100)
        // console.log(i.id)
      }
      return result;
    })
    ...
    return {state, submit, computedPrice}
  }
